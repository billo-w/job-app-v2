...
def get_ai_summary(query_details, total_jobs, job_listings_sample):
    """
    Calls the deployed Azure AI (OpenAI compatible) model to generate a
    recruiter-focused job market summary. Uses a sample of listings for the prompt.

    Args:
        query_details (dict): Contains 'what', 'where', 'country'.
        total_jobs (int): The total number of jobs found by Adzuna.
        job_listings_sample (list): A sample list of job listing dicts (first few results).
    ...
    # --- Prepare the prompt for the AI model (Recruiter Focus) ---
    # Use only the first 5-7 titles for the prompt to keep it concise
    ...
    app.logger.info(f"Sending recruiter-focused request to Azure AI Endpoint: {AZURE_AI_ENDPOINT}")
    try:
        response = requests.post(AZURE_AI_ENDPOINT, headers=headers, json=payload, timeout=25)
        ...
        if 'choices' in response_data and len(response_data['choices']) > 0:
            ...
            if message and 'content' in message:
                ...
                app.logger.info("Successfully received recruiter summary from Azure AI.")
...
def get_insights():
    ...
    params = {
        ...
        'what': job_title,
        'where': location,
        'results_per_page': RESULTS_PER_PAGE, # Use the updated variable
        'content-type': 'application/json'
    }
    ...
    try:
        ...
        response = requests.get(api_url, params=params, timeout=20) # Slightly increased timeout for more results
        response.raise_for_status()
        data = response.json()
        ...
        results = data.get('results', []) # This now contains up to RESULTS_PER_PAGE jobs
        job_listings = [ # Process all fetched jobs for display
            {
                "title": job.get('title'),
        ...
        insights_data = {
            ...
            "job_listings": job_listings, # Pass the full list for display
            "ai_summary": None
        }

        # --- Call Azure AI for Summary (if Adzuna call was successful) ---
        if insights_data:
            # Pass only the first few listings as a sample to the AI function
            ai_summary = get_ai_summary(
                insights_data['query'],
                insights_data['total_matching_jobs'],
                insights_data['job_listings'][:10] # Send sample (e.g., first 10) to AI
            )
            if ai_summary:
         ...
         try:
             ...
             if api_error_details: error_message += f" Details: {str(api_error_details)[:200]}"
         except Exception: pass
